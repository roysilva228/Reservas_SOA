...
export default function GestionarCanchasPage() {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]:
        name === 'precio_hora' || name === 'id_sede_fk'
          ? parseFloat(value)
          : value,
    });
  };
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      const sedeSeleccionada = sedes.find(
        (s) => s.id_sede === parseInt(formData.id_sede_fk)
      );
      ...
      const response = await axios.post(
        `${API_CANCHAS}/canchas/generar-descripcion-ia`,
        {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(
          `${API_CANCHAS}/canchas/${editId}`,
          dataToSend,
          config
        );
      } else {
        await axios.post(`${API_CANCHAS}/canchas/`, dataToSend, config);
      }
      closeModal();
      fetchData();
    } catch (err) {
  ...
  const handleDelete = async (id_cancha) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.delete(`${API_CANCHAS}/canchas/${id_cancha}`, config);
      fetchData();
    } catch (err) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Administra el inventario de espacios deportivos.
          </p>
        </div>
        <button
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    ...
                    <div className="text-xs text-gray-500">
                      {cancha.sede ? cancha.sede.nombre : 'Sin Sede'}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <select
                      ...
                      className="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 p-2 focus:ring-blue-500 border"
                      required
                    >
                ...
                <div>
                  <div className="flex justify-between items-end mb-1">
                    ...
                    <button
                      ...
                      className="text-xs bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-3 py-1 rounded-full hover:opacity-90 transition-opacity flex gap-1"
                    >
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows="3"
                    className="w-full rounded-md border-gray-300 p-2 border focus:ring-purple-500"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    URL de la Foto (Copia y pega de Google)
                  </label>
                  <input
                    type="text"
                    name="url_foto"
                    ...
                    onChange={handleChange}
                    className="w-full rounded-md border-gray-300 bg-gray-50 p-2 border focus:ring-blue-500"
                  ...
                  {formData.url_foto && (
                    <div className="mt-3 text-center">
                      ...
                      <img 
                        ...
                        alt="Vista previa" 
                        className="h-40 mx-auto object-cover rounded-lg shadow-md border border-gray-200"
