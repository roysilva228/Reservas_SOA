...
export default function RegisterPage() {
  ...
  const handleConsultarDNI = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`http://127.0.0.1:8000/usuarios/consultar-dni?dni=${dni}`);
      
      // (Opcional) Dejamos el console.log para futuras pruebas
      console.log("Respuesta de Decolecta:", response.data); 

      // --- ¡¡AQUÍ ESTÁ LA CORRECCIÓN!! ---
      // Usamos las claves correctas que vimos en la consola
  ...
  // ... El resto del return (el JSX) es idéntico ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-100">
      <div className="w-full max-w-lg rounded-lg bg-white p-8 shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                type="text"
                id="nombre"
                className="w-full rounded-md border border-gray-300 p-3 bg-gray-100 shadow-sm"
                value={nombre}
          ...
          {error && (
            <div className="rounded-md border border-red-400 bg-red-100 p-3 text-center text-sm text-red-700">
              {error}
            </div>
          )}
          ...
          <button
            ...
            className="w-full rounded-md bg-blue-600 p-3 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50"
          >
