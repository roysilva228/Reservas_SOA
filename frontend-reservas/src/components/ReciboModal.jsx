...
export default function ReciboModal({ reserva, onClose }) {
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF({
      ...
      unit: "mm",
      format: [80, 150] // Formato tipo Ticket (80mm ancho)
    });
    ...
    // Estado
    doc.setFontSize(8);
    doc.setTextColor(isConfirmed ? 34 : 202, isConfirmed ? 197 : 138, isConfirmed ? 94 : 4); // Verde o Amarillo
    ...
    doc.text("Cancha:", 5, y);
    doc.text(`#${reserva.id_cancha_fk}`, 75, y, { align: "right" }); // Ajusta si tienes el nombre
    y += 7;
    ...
    doc.text("Hora:", 5, y);
    doc.text(`${reserva.hora_inicio.slice(0,5)} - ${reserva.hora_fin.slice(0,5)}`, 75, y, { align: "right" });
    y += 10;
    ...
    doc.setFont("helvetica", "bold");
    doc.text("TOTAL", 5, y);
    doc.text(`S/. ${reserva.monto_pagado}`, 75, y, { align: "right" });
          ...
          <div className="bg-white border-2 border-dashed border-gray-300 rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-gray-800">{reserva.hora_inicio.slice(0,5)} - {reserva.hora_fin.slice(0,5)}</span>
              </div>
            </div>
            ...
            <div className="border-t border-gray-200 mt-4 pt-4 flex justify-between items-end">
              ...
              <span className="text-2xl font-extrabold text-blue-600">S/. {reserva.monto_pagado}</span>
            </div>
          </div>
        </div>
